...
export default function Chat() {
  ...
  return (
    <div className="bg-[#121212] h-screen flex flex-col">
      {/* Navigation bar */}
      <div className="bg-[#1f1f1f] w-full p-6 flex items-center text-white shadow-md">
        <img
      ...
      <div className="flex-grow overflow-y-auto p-4">
        {chatList.map((item, index) => (
          ...
          >
            <div
              className={`relative max-w-xs p-4 rounded-lg shadow-md ${item.senderUid === state.myUid ? 'bg-[#0078FF] text-white' : 'bg-[#333] text-white'} ${item.senderUid === state.myUid ? 'mr-2' : 'ml-2'}`}
              ...
              {showOptions === item.id && (
                <div className="absolute bottom-[-3rem] right-0 bg-[#444] border border-gray-500 rounded-lg shadow-lg z-10">
                  <button
                    ...
                    className="block w-full text-left px-4 py-2 text-sm text-yellow-500 hover:bg-[#c4bcbc]"
                  >
                    Edit
                  </button>
                  <button
                    ...
                    className="block w-full text-left px-4 py-2 text-sm text-red-500 hover:bg-[#131313]"
                  >
                    Delete
                  </button>
                </div>
      ...
      {/* Input section */}
      <div className="bg-[#1f1f1f] p-4 flex items-center border-t border-gray-700">
        <input
          value={messages}
        ...
        <button
          onClick={sendMessage}
          className="ml-2 py-2 px-4 bg-[#0078FF] text-white rounded-lg hover:bg-[#0056b3]"
        >
          Send
        </button>
      </div>
